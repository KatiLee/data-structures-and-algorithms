...
const addQuestion = (arr) => {
  // Solution code here...
  let curiousArray = arr.map(string => (string + '?'));
  return curiousArray;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    twoToArray.push(Math.pow(2, arr[i]));
  };
  return twoToArray;
};
...
const forEachTwoToThe = (arr) => {
  ...
  let newTwoToArray = [];
  arr.forEach(number => {
    newTwoToArray.push(Math.pow(2, number));
  });
  return newTwoToArray;
};
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let brandNewTwoToArray = arr.map(number => (Math.pow(2, number)));
  return brandNewTwoToArray;
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 7 - Stretch Goal

Write a function named charCode that, given an array of letters as an input, uses map to return a new array where each 
element is the result of the `charCodeAt` method on the original array element.
...
Write a function that, given an array of numbers as input, uses map to return a new array where each element is either the 
