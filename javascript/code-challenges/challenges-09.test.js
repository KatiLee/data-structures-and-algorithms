...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
 const hasKid =arr.find(char => char.name === character);
 if (!hasKid || !hasKid.children)return false;
return Object.values(hasKid.children).length > 0;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  const hasKid = arr.find(char => char.name === character);
  if (!hasKid || !hasKid.children) return false;
  return Object.entries(hasKid.children).length > 0;
};
